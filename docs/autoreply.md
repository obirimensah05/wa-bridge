...
This repo includes a local autoreply service that sits beside `wa-bridge`.

## What it does now

- runs a local HTTP service for autoreply control
- accepts immediate webhook calls from `wa-bridge`
- builds a style corpus from outbound WhatsApp history plus second-brain content
- generates draft replies in Obiri's voice with Claude CLI
- sends live draft notifications to Telegram
- can auto-send through the local `wa-bridge` API when policy + safety checks allow it
...
- webhook delivery from `wa-bridge` into the autoreply service
...
AUTOREPLY_SECOND_BRAIN_ROOT=/Users/obirimensah/Apps/onyankopon-second-brain-sync
...
AUTOREPLY_WA_API_TOKEN=<wa-bridge bearer token>
...
# wa-bridge -> autoreply webhook wiring
...
curl -X PUT \
  ...
  -d '{
    "mode": "auto",
    "scope": "contacts",
    "contacts": ["13312055999@s.whatsapp.net"],
    "groups": [],
...
This is the route `wa-bridge` calls for inbound events. It now accepts both real message payloads and `/v1/webhook/test` test pings from the bridge.
