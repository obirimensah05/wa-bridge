...
export const WA_BRIDGE_BASE = (process.env.WA_BRIDGE_BASE?.trim() || 'http://127.0.0.1:8080').replace(/\/$/, '')
export const WA_BRIDGE_TOKEN = process.env.WA_BRIDGE_TOKEN?.trim() || process.env.API_TOKEN?.trim() || null
...
export const AUTOREPLY_SECOND_BRAIN_ROOT = process.env.AUTOREPLY_SECOND_BRAIN_ROOT?.trim()
  || '/Users/obirimensah/Apps/onyankopon-second-brain-sync'
