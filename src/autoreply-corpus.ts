...
async function buildSecondBrainSection(noteLimit: number): Promise<string> {
  ...
  const url = `${env.SUPABASE_URL}/rest/v1/onyankopon_documents?select=notion_id,title,last_edited_time,url,body_text&order=last_edited_time.desc.nullslast&limit=${noteLimit}`
